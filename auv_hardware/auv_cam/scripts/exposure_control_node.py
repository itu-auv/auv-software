...
The RoI is a constant normalized [x, y, w, h] param for now; a
sensor_msgs/RegionOfInterest topic (~roi topic, pixel units) overrides it
while fresh, for later wiring to a detector.
...
class ExposureControlNode:
    def __init__(self):
        ...
        rospy.Subscriber("~roi", RegionOfInterest, self.roi_cb, queue_size=1)
        rospy.Subscriber(
    ...
    def roi_cb(self, msg):
        self.roi_msg = msg
        self.roi_msg_stamp = rospy.Time.now()

    def current_roi(self, w, h):
        """Pixel-space (x0, y0, x1, y1), topic override wins while fresh."""
        m = self.roi_msg
        if (
            m is not None
            and (rospy.Time.now() - self.roi_msg_stamp).to_sec() < self.roi_timeout
        ):
            x0, y0 = m.x_offset, m.y_offset
    ...
    def image_cb(self, msg):
        ...
        if self.pub_debug is not None and self.pub_debug.get_num_connections() > 0:
            self.publish_debug(gray, (x0, y0, x1, y1), pct, metric)
    ...
    def publish_debug(self, gray, box, pct, metric):
