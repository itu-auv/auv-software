...
class PointCloudProcessor:
    ...
    def filter_points_in_roi(self, points, point_colors=None):
        """Filter points to keep only those within YOLO detection bounding boxes"""
        if not self.current_detections:
            return np.array([]), np.array([]) if point_colors is not None else np.array([])

        # Calculate distances from origin (camera)
        distances = np.linalg.norm(points, axis=1)
        # Create initial mask for points within max_distance
        mask = distances < self.max_distance

        # Create mask for points within any detection bbox
        ...
        for detection in self.current_detections:
            ...
            # Assuming points are in camera frame and using simple perspective projection
            # Note: This is a simplified projection, might need adjustment based on your camera calibration
            points_2d = points[mask] / distances[mask, np.newaxis]  # Normalize by depth
            points_2d = points_2d[:, :2]  # Keep only x,y components
            
            # Scale to image coordinates (assuming normalized coordinates)
            points_2d[:, 0] = (points_2d[:, 0] + 1) * 0.5  # Scale from [-1,1] to [0,1]
            points_2d[:, 1] = (points_2d[:, 1] + 1) * 0.5
            
            # Check which points fall within the bbox
            in_bbox = (points_2d[:, 0] >= x) & (points_2d[:, 0] <= x + width) & \
                     (points_2d[:, 1] >= y) & (points_2d[:, 1] <= y + height)
            
            roi_mask[mask] |= in_bbox

        # Combine distance and ROI masks
        final_mask = mask & roi_mask

        if point_colors is not None:
            return points[final_mask], point_colors[final_mask]
    ...
    def yolo_callback(self, msg):
        ...
        self.current_detections = msg.detections.detections
    ...
    def pointcloud_callback(self, msg):
        ...
        # Filter points to keep only those within ROIs
        filtered_points = self.filter_points_in_roi(points)
        
        if len(filtered_points) < self.min_samples:
            rospy.logwarn("Not enough points in ROI after filtering")
            return
