...
class ReferenceVectorGeneratorNode:
    def __init__(self):
        ...
        self.model_path = rospy.get_param(
            "~model_path",
            os.path.expanduser(
                "~/AUTO_LABEL/auv_detection/models/dinov3_vits16_pretrain_lvd1689m-08c60483.pth"
            ),
        )
        ...
        self.image_sub = message_filters.Subscriber(
            "/taluy/vision/draw_mask/image", ROSImage
        )
        self.mask_sub = message_filters.Subscriber(
            "/taluy/vision/draw_mask/mask", ROSImage
        )
        ...
        self.srv = rospy.Service(
            "save_reference_vector", Trigger, self.handle_save_reference_vector
        )

        rospy.loginfo(
            "[ReferenceVectorGeneratorNode] Subscribed to /taluy/vision/draw_mask/image and /taluy/vision/draw_mask/mask"
        )
        rospy.loginfo(
            f"[ReferenceVectorGeneratorNode] Vectors directory: {self.vectors_dir}"
        )
        rospy.loginfo(
            "[ReferenceVectorGeneratorNode] Service 'save_reference_vector' is ready."
        )
    ...
    def handle_save_reference_vector(self, req):
        with self.lock:
            if self.latest_image is None or self.latest_mask is None:
                return TriggerResponse(
                    success=False,
                    message="No mask and image pair has been received yet.",
                )
        ...
        try:
            ...
            if np.sum(mask) == 0:
                return TriggerResponse(
                    success=False,
                    message="The received mask is empty! Paint a region first.",
                )
            ...
            with torch.inference_mode():
                feats = self.model.get_intermediate_layers(
                    transformed_frame.unsqueeze(0), n=1, reshape=True
                )[0]
            ...
            first_frame_mask = F.one_hot(
                interpolated_mask.long(), num_classes=2
            ).float()
