...
class CameraRollStabilizer:
    ...
    def __init__(self):
        ...
        # If no synced (odom+image) frame arrives within this window, publish a
        # placeholder image telling the operator to start localization.
        ...
        # Standalone image subscriber for fallback placeholder publishing.
        self.fallback_sub = rospy.Subscriber(
    ...
    def fallback_callback(self, img_msg: Image):
        """Publish a placeholder image if no synced odom+image pair arrived recently."""
        now = rospy.Time.now()
        if (now - self.last_sync_time).to_sec() < self.odom_timeout:
            return
        ...
        try:
            ...
            rospy.logwarn_throttle(
                5.0, "[camera_roll_stabilizer] fallback decode failed: %s", str(e)
            )
            return
        ...
        try:
            ...
            rospy.logwarn_throttle(
                5.0, "[camera_roll_stabilizer] fallback encode failed: %s", str(e)
            )
