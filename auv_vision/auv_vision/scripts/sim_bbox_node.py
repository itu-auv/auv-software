...
For the bottom_seg camera, segmentation masks are generated either by
projecting mesh triangle faces or by rasterising configured boundary polygons.
...
def render_boundary_mask(
    ...
    us = fx * points_camera[:, 0] / depths + cx
    vs = fy * points_camera[:, 1] / depths + cy
    polygon = np.rint(np.column_stack((us, vs))).astype(np.int32)
    ...
    if (
        hull_us.max() < 0
        or hull_us.min() >= image_w
        or hull_vs.max() < 0
        or hull_vs.min() >= image_h
    ):
        return None
    ...
    u_min_all = float(hull_us.min())
    u_max_all = float(hull_us.max())
    v_min_all = float(hull_vs.min())
    v_max_all = float(hull_vs.max())
    bcx = (u_min_all + u_max_all) / 2.0
    bcy = (v_min_all + v_max_all) / 2.0
    return mask, (bcx, bcy, u_max_all - u_min_all, v_max_all - v_min_all)
    ...
    def _image_cb(self, msg: Image):
        ...
        for obj in self.objects:
            ...
            for model_name in model_names:
                ...
                elif obj.render_mask:
                    if obj.boundary_h is None:
                        continue
                    result = render_boundary_mask(
                        obj.boundary_h,
                        full_tf,
                        self.intrinsics,
                        self.image_w,
        ...
        if "mesh" in obj_cfg:
            ...
            all_objects.append(
                SimObject(
                    ...
                    mesh=mesh,
                    render_mask=obj_cfg.get("render_mask", False),
