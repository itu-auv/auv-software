...
class DrawMaskNode:
    ...
    def handle_draw_mask_service(self, req):
        with self.image_lock:
            if self.latest_msg is None:
                err_msg = (
                    f"No bottom camera image received yet on {self.input_image_topic}."
                )
        ...
        rospy.loginfo(
            "[DrawMaskNode] Service triggered. Opening drawing GUI in main thread..."
        )
        ...
        if self.gui_status == "saved":
            ...
            return TriggerResponse(
                success=False, message="Mask drawing cancelled by user."
            )
        else:
            return TriggerResponse(
                success=False, message="An error occurred during GUI drawing."
            )

    def run_gui_loop(self):
        try:
            frame = self.bridge.imgmsg_to_cv2(
                self.gui_input_msg, desired_encoding="bgr8"
            )
        ...
        while not rospy.is_shutdown():
            ...
            if key == ord("a") or key == ord("s"):
                if np.sum(mask) == 0:
                    rospy.logwarn(
                        "[DrawMaskNode] Mask is empty! Please paint a region first."
                    )
                    continue
                ...
                self.publish_mask_and_image(
                    self.saved_frame, self.saved_mask, self.saved_header
                )
    ...
    def publish_mask_and_image(self, frame, mask, header):
        ...
        rospy.loginfo_throttle(
            5.0, "[DrawMaskNode] Continuously publishing saved mask and image..."
        )
