...
#   K from `heatmaps`, C from `mask_logits`, objectness from `logits`; the
#   only non-shape value, mask_threshold, comes from the YAML (default 0.5).
#
...
def engine_meta(engine):
    ...
    engine has a `heatmaps` output (K = its channels, C = `mask_logits`
    channels or 0), 'objectness' when it has `logits` (1 channel)."""
    outs = {name: tuple(t.shape) for name, t in engine.outputs.items()}
    _, _, h, w = engine.input_shape
    if TRT_OUT_HEATMAPS in outs:
        masks = outs.get(TRT_OUT_MASKS)
        return dict(
            kind="joint",
            img_size=[h, w],
            num_kps=int(outs[TRT_OUT_HEATMAPS][1]),
    ...
    raise ValueError(
        ...
        f"(joint) or {TRT_OUT_LOGITS!r} (objectness); rebuild with vitpose_export.py"
    )
...
class VitposeTRT(VitposeModel):
    ...
    engine's IO tensors (engine_meta); mask_threshold from the YAML (0.5 when
    null — an engine has no training config to fall back on). heatmaps /
    ...
    def __init__(
        ...
        self._init_meta(
            active_heads=["pose"] + (["seg"] if meta["num_masks"] else []),
        ...
        self.model = None
        self._out_heatmaps = TRT_OUT_HEATMAPS
