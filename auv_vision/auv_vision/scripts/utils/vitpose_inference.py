...
class JointVitpose(nn.Module):
    """ViT trunk + optional pose head + optional mask head (at least one);
    attribute names match the checkpoint state dict (`backbone.* /
    pose_head.* / mask_head.*`). A seg-only checkpoint (active_heads
    ["seg"], e.g. the pipe/track model) has no pose head: forward returns
    heatmaps=None."""

    def __init__(self, backbone_cfg, pose_head_cfg=None, mask_head_cfg=None):
        super().__init__()
        if not pose_head_cfg and not mask_head_cfg:
            raise ValueError("JointVitpose needs a pose head or a mask head")
        self.backbone = ViT(**backbone_cfg)
        self.pose_head = (
            TopdownHeatmapSimpleHead(**pose_head_cfg) if pose_head_cfg else None
        )
        self.mask_head = (
    ...
    def forward(self, x):
        ...
        heatmaps = self.pose_head(features) if self.pose_head is not None else None
        mask_logits = None
...
class VitposeModel:
    ...
    def __init__(
        ...
        pose_cfg = (
            _head_cfg_from_state(state, "pose_head", embed_dim)
            if "pose" in self.active_heads
            else None
        )
        mask_cfg = (
        ...
        self._init_heads(
            pose_cfg["out_channels"] if pose_cfg else 0,
    ...
    def predict(self, img_rgb, bbox_xywh):
        ...
        if self.flip_tta:
            ...
            if heatmaps is not None and hm_f is not None:
        ...
        else:  # seg-only checkpoint: K=0
            kps = np.zeros((1, 0, 2), dtype=np.float32)
            scores = np.zeros((1, 0, 1), dtype=np.float32)
...
class VitposeTRT(VitposeModel):
    ...
    def _forward(self, tensor):
        ...
        heatmaps = outs[self._out_heatmaps].float() if self._out_heatmaps else None
