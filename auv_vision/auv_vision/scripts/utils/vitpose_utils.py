...
# `camera` required, `detection` + `model` + `process` optional (no
# `detection:` = the joint model eats the full frame at `rate` unbounded;
# no `model:` = detect-only).
...
def load_object_config(name_or_path: str, ns: str = "taluy") -> dict:
    ...
    required set minimal (`object`, `camera`). A missing `detection:` means
    full-frame inference (bbox_provider full_frame, no rate cap) — the shape
    of a seg-only model like pipe. The `camera` token is expanded into every
    camera-derived key (apply_camera)."""
    path = resolve_config_path(name_or_path)
    with open(path, "r") as handle:
        config = yaml.safe_load(handle)
    for section in ("object", "camera"):
        if section not in config:
            raise ValueError(f"{path}: missing required key '{section}'")
    if config.get("detection") is None:
        config["detection"] = {}
    config["_path"] = path
