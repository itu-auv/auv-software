...
def export(ckpt, out_dir, opset, check):
    ...
    if "active_heads" in payload:
        ...
        kind = "joint"
        outputs = ["heatmaps"] + (["mask_logits"] if runner.num_masks else [])
        meta = dict(
            kind=kind,
            ...
            outputs=dict(
                {"heatmaps": "heatmaps"},
                **({"mask_logits": "mask_logits"} if runner.num_masks else {}),
            ),
        )
        model = runner.model
        if not runner.num_masks:
            # ONNX can't export a None output: wrap to a single tensor.
            class PoseOnly(torch.nn.Module):
                def __init__(self, m):
                    super().__init__()
                    self.m = m
