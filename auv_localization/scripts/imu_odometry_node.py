...
import numpy as np
import yaml
from auv_localization.srv import CalibrateIMU, CalibrateIMUResponse  # Replace 'my_package' with your actual package name
...
class ImuToOdom:
    def __init__(self):
        ...
        self.imu_subscriber = rospy.Subscriber("sensors/imu/data", Imu, self.imu_callback)
        self.odom_publisher = rospy.Publisher("localization/odom_imu", Odometry, queue_size=10)
        ...
        self.calibration_service = rospy.Service('calibrate_imu', CalibrateIMU, self.calibrate_imu)
        
    def imu_callback(self, imu_msg):
        if self.calibrating:
            self.calibration_data.append([imu_msg.angular_velocity.x, imu_msg.angular_velocity.y, imu_msg.angular_velocity.z])
        ...
        self.odom_msg.pose.covariance[:9] = list(imu_msg.orientation_covariance)  # Only the first 9 elements for orientation
        ...
        self.odom_msg.twist.covariance[21:30] = list(imu_msg.angular_velocity_covariance)  # Angular velocity covariance in the correct position
    ...
    def save_calibration_data(self):
        ...
        config_path = rospy.get_param('~config_path', 'config/calibration_data.yaml')
