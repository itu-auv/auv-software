...
class PressureToOdom:
    def __init__(self):
        ...
        self.depth_subscriber = rospy.Subscriber("sensors/external_pressure_sensor/depth", Float32, self.depth_callback)
        self.odom_publisher = rospy.Publisher("localization/odom_pressure", Odometry, queue_size=10)
    ...
    def load_calibration_data(self):
        config_path = rospy.get_param('~config_path', 'config/pressure_calibration.yaml')
        try:
            with open(config_path, 'r') as f:
                calibration_data = yaml.safe_load(f)
                self.odom_msg.pose.covariance[14] = calibration_data.get('pose_position_z_covariance', 0.0)
                rospy.loginfo("Calibration data loaded. Pose position Z covariance: {}".format(self.odom_msg.pose.covariance[14]))
