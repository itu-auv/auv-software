...
class ObjectPositionEstimator:
    def __init__(self):
        ...
        self.id_tf_map = {9: "bin/whole", 10: "bin/red", 11: "bin/blue"}

        # Subscriptions
        self.yolo_sub = rospy.Subscriber("/yolo_result", YoloResult, self.yolo_callback)
        self.altitude_sub = rospy.Subscriber(
            "/taluy/sensors/dvl/altitude", Float32, self.altitude_callback
        )
    ...
    def yolo_callback(self, msg):
        if (
            self.camera_width is None
            or self.camera_height is None
            or self.altitude is None
        ):
            return
