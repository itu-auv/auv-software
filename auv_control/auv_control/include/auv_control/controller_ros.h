...
class ControllerROS {
 public:
  ...
  void spin() {
    ...
    while (ros::ok()) {
      ...
      // In case velocity commands are timeouted, pretend zero velocity is
      // desired
      if (is_cmd_vel_timeouted()) {
        desired_state_.tail(6).setZero();
      }

      ROS_DEBUG_STREAM("Desired state: " << desired_state_.transpose());
 ...
 private:
  ...
  bool is_cmd_vel_timeouted() const {
    return (ros::Time::now() - latest_cmd_vel_time_).toSec() > 0.4;
  }

  // cmd_pose timeout is here but not cared.
  bool is_cmd_pose_timeouted() const {
    return (ros::Time::now() - latest_cmd_pose_time_).toSec() > 0.4;
  }
