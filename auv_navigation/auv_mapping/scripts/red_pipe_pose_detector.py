...
class RedPipePoseDetector:
    def __init__(self):
        ...
        # Parameters
        self.pipe_diameter = rospy.get_param('~pipe_diameter', 0.0127)  # 5cm varsayılan PVC boru çapı
        self.min_contour_area = rospy.get_param('~min_contour_area', 500)
        self.max_contour_area = rospy.get_param('~max_contour_area', 50000)
        self.safety_margin = rospy.get_param('~safety_margin', 0.2)  # 20cm güvenlik mesafesi
        ...
        self.debug_image_pub = rospy.Publisher('/red_pipes/debug_image', Image, queue_size=1)
    ...
    def draw_debug_info(self, image, poses_info):
        ...
        for pose, center, radius, distance in poses_info:
            ...
            # 3D koordinatlar
            pos_text = f"({pose.position.x:.2f}, {pose.position.y:.2f}, {pose.position.z:.2f})"
            cv2.putText(debug_image, pos_text, (center[0] - 80, center[1] + radius + 20),
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            pose_array.header.frame_id = msg.header.frame_id if msg.header.frame_id else "camera_link"
