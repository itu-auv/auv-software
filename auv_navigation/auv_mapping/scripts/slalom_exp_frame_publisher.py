...
class SlalomExpFramePublisher:
    def __init__(self):
        ...
        self.srv_publish_search_points = rospy.Service("slalom/publish_search_points", Trigger, self.publish_search_points_callback)
        self.srv_start_point_search = rospy.Service("slalom/start_point_search", SetBool, self.start_point_search_callback)
        self.srv_stop_point_search = rospy.Service("slalom/stop_point_search", SetBool, self.stop_point_search_callback)
        self.srv_publish_waypoints = rospy.Service("slalom/publish_waypoints", SetBool, self.publish_waypoints_callback)
    ...
    def publish_search_points_callback(self, req):
        try:
            for c in ["start", "left", "right"]:
                ...
                t_odom.transform.translation = Vector3(pose_in_odom.pose.position.x, pose_in_odom.pose.position.y, pose_in_odom.pose.position.z)
    ...
    def start_point_search_callback(self, req):
        if req.data:
            ...
            self.collecting = False
            return SetBoolResponse(success=True, message="Stopped point search (collection paused)")

    def stop_point_search_callback(self, req):
        self.collecting = False
        rospy.loginfo(f"Stopped point collection. Collected {len(self.points)} points. Filtering...")
        self.filter_points()
        ...
        return SetBoolResponse(success=True, message=f"Processed and published {len(self.tfs) if self.tfs else 0} centroids")
    ...
    def publish_waypoints_callback(self, req):
        ...
        for t in self.tfs:
            ...
            if idx not in groups:
                groups[idx] = {}
            groups[idx][pt_type] = np.array([t.transform.translation.x, t.transform.translation.y])
        ...
        for idx, g in groups.items():
            for w in ["left", "right"]:
                ...
                trans = self.tf_buffer.lookup_transform("odom", self.base_link_frame, rospy.Time(0), rospy.Duration(1.0))
                q_base = [trans.transform.rotation.x, trans.transform.rotation.y, trans.transform.rotation.z, trans.transform.rotation.w]
                # ????
    ...
    def yolo_callback(self, msg):
        ...
        for x in detections.detections:
            ...
            transform_stamped_msg.header.frame_id = self.base_link_frame + "/front_camera_optical_link_stabilized"
