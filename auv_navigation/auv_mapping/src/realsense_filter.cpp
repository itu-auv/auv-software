...
class PointCloudFilterNode {
 ...
 private:
  ...
  void cloudCallback(const sensor_msgs::PointCloud2::ConstPtr& cloud_msg) {
    ...
    refraction_corrected_cloud->header =
        pcl_cloud->header;  // Copy header from original PCL cloud

    for (const auto& pt_in : pcl_cloud->points) {
      double x = pt_in.x;
      double y = pt_in.y;
      double z_in = pt_in.z;
      double corrected_z = z_in;  // Default to original z if correction is not
                                  // possible/applicable

      // Using epsilon for floating point comparisons to avoid division by zero
      // for r
      double r = std::sqrt(x * x + z_in * z_in);

      if (r > std::numeric_limits<double>::epsilon()) {
        double val_for_acos = z_in / r;
        // Clamp val_for_acos to [-1.0, 1.0] to prevent acos domain errors due
        // to precision issues
        ...
        double angle_deg =
            theta * 180.0 / M_PI;  // M_PI should be defined in <cmath>
      ...
      if (std::isfinite(corrected_pt.x) && std::isfinite(corrected_pt.y) &&
          std::isfinite(corrected_pt.z)) {
        refraction_corrected_cloud->points.push_back(corrected_pt);
      }
    }

    refraction_corrected_cloud->width =
        refraction_corrected_cloud->points.size();
    refraction_corrected_cloud->height = 1;
    // is_dense is true if all points are finite. Since we check std::isfinite
    // before adding, all points in refraction_corrected_cloud are finite.
    ...
    temp_cloud->header = refraction_corrected_cloud
                             ->header;  // Use header from the corrected cloud
    ...
    for (const auto& pt :
      ...
      if (pt.y <= altitude_ - 0.45 && pt.y >= depth_value_ + 0.30 &&
          pt.z < 5.0) {
    ...
    temp_cloud->height = 1;
    temp_cloud->is_dense =
        false;  // Keep original behavior for the final published cloud
  ...
  // Removed getMinDepth and getDepthValue functions as they are no longer
  // needed
};
