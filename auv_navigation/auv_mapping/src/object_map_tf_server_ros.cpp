...
bool ObjectMapTFServerROS::set_transform_handler(
  ...
  const auto static_transform = transform_to_static_frame(req.transform, req.transform.header.stamp);
...
ObjectMapTFServerROS::transform_to_static_frame(
  ...
  const auto static_to_parent_transform =
      get_transform(static_frame_, parent_frame, lookup_time, ros::Duration(4.0));
