...
"""
    ...
    def _expire_old_keys(self) -> None:
        ...
        now = time.time()
        expired = [
            k for k, t in self.key_timestamps.items() if now - t > KEY_TIMEOUT_SEC
        ]
        for k in expired:
    ...
    def _print_status(self, force: bool = False) -> None:
        ...
        active = (
            ", ".join(sorted(self.key_timestamps.keys()))
    ...
    def run(self) -> None:
        ...
        try:
            ...
            while not rospy.is_shutdown():
                ...
                self.current_twist = (
                    self._compute_twist() if self.driving_enabled else Twist()
                )
