...
class RotationKdeState(RotationState):
    def __init__(
        self,
        source_frame,
        look_at_frame,
        ...
        super().__init__(
            ...
            full_rotation_timeout=full_rotation_timeout,
            rate_hz=rate_hz,
...
class SetFrameLookingAtKdeState(SetFrameLookingAtState):
    ...
    def execute(self, userdata):
        ...
        while not rospy.is_shutdown() and rospy.Time.now() < end_time:
            ...
            active_frame = self.look_at_frame
            try:
                lookup_fresh_transform(
                    self.tf_buffer,
                    self.source_frame,
                ...
                active_frame = self.fallback_frame
            ...
            try:
                ...
                rospy.loginfo_throttle(
                    1.0,
                    f"Looking at {active_frame} (dynamic target). Time remaining: {time_remaining:.2f}s",
                )
        ...
        rospy.loginfo(
            f"Successfully looked at {self.look_at_frame} (or fallback) for {self.duration_time} seconds"
        )
        return "succeeded"
...
class SearchForPropKdeState(smach.StateMachine):
    """
    1. RotationKdeState: Rotates to find a prop's frame (looks for primary _kde or fallback p_).
    2. SetAlignControllerTargetState: Sets the align controller target.
    3. SetFrameLookingAtKdeState: Sets a target frame's pose based on looking at active frame.
    ...
    def __init__(
        ...
        fallback_frame = look_at_frame
        if look_at_frame.endswith("_kde"):
            base_name = look_at_frame[:-4]
            fallback_frame = f"p_{base_name}"

        duration_time = timeout if set_frame_duration is None else set_frame_duration
        rospy.loginfo(
            "SearchForPropKdeState: Initiated with look_at_frame='%s', fallback_frame='%s'",
            look_at_frame,
            fallback_frame,
        )
